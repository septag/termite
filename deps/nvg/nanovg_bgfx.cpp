...
#include "shaders_h/nanovg_fill.fso"
	...
	static int glnvg__deleteTexture(struct GLNVGcontext* gl, int id)
	{
		...
		{
			if (gl->textures[ii].id.value == id)
			{
				if (gl->textures[ii].id.isValid()
	...
	static int nvgRenderCreate(void* _userPtr)
	{
		...
		gl->prog = gl->driver->createProgram(
						  gl->driver->createShader(vs_nanovg_fill)
	...
	static int nvgRenderCreateTexture(void* _userPtr, int _type, int _width, int _height, int _flags, const unsigned char* _rgba)
	{
		...
		return tex->id.isValid() ? tex->id.value : 0;
	}
	...
	static int nvgRenderUpdateTexture(void* _userPtr, int image, int x, int y, int w, int h, const unsigned char* data)
	{
		...
		gl->driver->updateTexture2D(tex->id
				...
				, w
				, h
				, gl->driver->copy(data + y*pitch + x*bytesPerPixel, h*pitch)
				, pitch
				);
	...
	static void nvgRenderSetUniforms(struct GLNVGcontext* gl, int uniformOffset, int image)
	{
		...
		gl->driver->setUniform(gl->u_scissorMat, tmp, 1);
		glnvg__mat3(tmp, frag->paintMat);
		gl->driver->setUniform(gl->u_paintMat, tmp, 1);

		gl->driver->setUniform(gl->u_innerCol,        frag->innerCol.rgba, 1);
		gl->driver->setUniform(gl->u_outerCol,        frag->outerCol.rgba, 1);
		gl->driver->setUniform(gl->u_scissorExtScale, &frag->scissorExt[0], 1);
		...
		if (image != 0)
		{
			...
			if (tex != NULL)
			{
				...
				if (gl->u_halfTexel.isValid())
				{
					float halfTexel[4] = { 0.5f / tex->width, 0.5f / tex->height };
					gl->driver->setUniform(gl->u_halfTexel, halfTexel, 1);
				}
			}
		}
	...
	static void fan(GLNVGcontext* gl, uint32_t _start, uint32_t _count)
	{
		...
		gl->driver->setTransientIndexBuffer(&tib);
	}
	...
	static void nvgRenderFlush(void* _userPtr)
	{
		...
		if (gl->ncalls > 0)
		{
			gl->driver->allocTransientVertexBuffer(&gl->tvb, gl->nverts, s_nvgDecl);
...
// 			else
	...
	static void nvgRenderDelete(void* _userPtr)
	{
		...
		gl->driver->destroyProgram(gl->prog);
		gl->driver->destroyTexture(gl->texMissing);

		gl->driver->destroyUniform(gl->u_scissorMat);
		gl->driver->destroyUniform(gl->u_paintMat);
		gl->driver->destroyUniform(gl->u_innerCol);
		gl->driver->destroyUniform(gl->u_outerCol);
		gl->driver->destroyUniform(gl->u_viewSize);
		gl->driver->destroyUniform(gl->u_scissorExtScale);
		gl->driver->destroyUniform(gl->u_extentRadius);
		gl->driver->destroyUniform(gl->u_params);
		gl->driver->destroyUniform(gl->s_tex);

		if (gl->u_halfTexel.isValid())
		{
			gl->driver->destroyUniform(gl->u_halfTexel);
		}

		for (uint32_t ii = 0, num = gl->ntextures; ii < num; ++ii)
		{
			if (gl->textures[ii].id.isValid() && (gl->textures[ii].flags & NVG_IMAGE_NODELETE) == 0)
			{
				gl->driver->destroyTexture(gl->textures[ii].id);
			}
		}
