...
{
    namespace gfx {
        TEE_API VertexDecl* beginDecl(VertexDecl* vdecl, RendererType::Enum _type = RendererType::Noop);
        TEE_API void endDecl(VertexDecl* vdecl);
        TEE_API VertexDecl* addAttrib(VertexDecl* vdecl, VertexAttrib::Enum _attrib, uint8_t _num, VertexAttribType::Enum _type,
                                      bool _normalized = false, bool _asInt = false);
        TEE_API VertexDecl* skipAttrib(VertexDecl* vdecl, uint8_t _numBytes);
        TEE_API void decodeAttrib(VertexDecl* vdecl, VertexAttrib::Enum _attrib, uint8_t* _num, VertexAttribType::Enum* _type,
                                  bool* _normalized, bool* _asInt);
        TEE_API bool hasAttrib(VertexDecl* vdecl, VertexAttrib::Enum _attrib);
        TEE_API uint32_t getDeclSize(const VertexDecl* vdecl, uint32_t _num);
    }
    ...
    public:
        ...
        explicit CVertexDecl(VertexDecl* decl) :
            m_decl(decl)
        {
        }

        inline CVertexDecl& begin(RendererType::Enum _type = RendererType::Noop)
        {
            gfx::beginDecl(m_decl, _type);
            return *this;
        }
        ...
        inline CVertexDecl& add(VertexAttrib::Enum attrib, uint8_t num, VertexAttribType::Enum type,
                                bool normalized = false, bool asInt = false)
        {
            gfx::addAttrib(m_decl, attrib, num, type, normalized, asInt);
            return *this;
        }

        inline CVertexDecl& skip(uint8_t numBytes)
        {
            gfx::skipAttrib(m_decl, numBytes);
            return *this;
        }
        ...
        inline void decode(VertexAttrib::Enum attrib, uint8_t* num, VertexAttribType::Enum* type, bool* normalized,
                           bool* asInt) const
        {
            gfx::decodeAttrib(m_decl, attrib, num, type, normalized, asInt);
        }

        inline bool has(VertexAttrib::Enum attrib) const
        {
            return gfx::hasAttrib(m_decl, attrib);
        }

        inline uint32_t getSize(uint32_t num) const
        {
            return gfx::getDeclSize(m_decl, num);
        }
    ...
    public:
        ...
        // Index buffers
        IndexBufferHandle(*createIndexBuffer)(const GfxMemory* mem, GfxBufferFlag::Bits flags/* = GpuBufferFlag::None*/);
        DynamicIndexBufferHandle(*createDynamicIndexBuffer)(uint32_t num, GfxBufferFlag::Bits flags/* = GpuBufferFlag::None*/);
        DynamicIndexBufferHandle(*createDynamicIndexBufferMem)(const GfxMemory* mem, GfxBufferFlag::Bits flags/* = GpuBufferFlag::None*/);
        ...
        uint32_t(*readTexture)(TextureHandle handle, void* data, uint8_t mip/* = 0*/);
