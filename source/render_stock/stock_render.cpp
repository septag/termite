...
static result_t initRenderer(bx::AllocatorI* alloc, GfxDriverApi* driver)
{
    ...
    driver->reset(conf.gfxWidth, conf.gfxHeight, conf.gfxDriverFlags);
...
PluginDesc* getStockRendererDesc()
{
    static PluginDesc desc;
    strcpy(desc.name, "StockRenderer");
    strcpy(desc.description, "");
    desc.type = PluginType::Renderer;
    desc.version = T_MAKE_VERSION(0, 1);
    return &desc;
}

void* initStockRenderer(bx::AllocatorI* alloc, GetApiFunc getApi)
{
