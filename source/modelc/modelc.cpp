...
static mtx4x4_t convertMtx(const aiMatrix4x4& m, ZAxis zaxis)
{
    switch (zaxis) {
    case ZAxis::Unknown:
        return mtx4x4_t(m.a1, m.b1, m.c1,
                        m.a2, m.b2, m.c2,
                        m.a3, m.b3, m.c3,
                        m.a4, m.b4, m.c4);
    case ZAxis::GL:
    ...
    case ZAxis::Up:
    {
        mtx4x4_t zup(1.0f, 0, 0,
                     0, 0, 1.0f,
                     0, 1.0f, 0,
                     0, 0, 0);
        mtx4x4_t r(m.a1, m.b1, -m.c1,
                   m.a2, m.b2, -m.c2,
                   -m.a3, -m.b3, m.c3,
                   m.a4, m.b4, -m.c4);
        mtx4x4_t res;
...
static aabb_t calcGeoBoundsNoSkin(const ModelData::Geometry& geo)
{
    ...
    for (int i = 0; i < geo.g.numVerts; i++) {
        ...
        aabbPushPoint(&bb, vec3_t(poss[i * 3], poss[i * 3 + 1], poss[i * 3 + 2]));
    }
    return bb;
}
...
static aabb_t calcGeoBoundsSkin(const ModelData::Geometry& geo)
{
    ...
    for (int i = 0; i < geo.g.numVerts; i++) {
        ...
        vec3_t pos(poss[i * 3], poss[i * 3 + 1], poss[i * 3 + 2]);
        vec3_t skinned(0, 0, 0);
...
static bool importModel(const Config& conf)
{
    ...
    ModelData model;
    mtx4x4_t rootMtx;   rootMtx.setIdentity();
