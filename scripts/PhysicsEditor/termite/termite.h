...
{% else %}
static const tee::vec2_t k{{ body.name }}Verts{{ forloop.counter }}[] = {{% for point in fixture.hull %} {% if not forloop.first %}, {% endif %}termite::vec2({{ point.x|floatformat:2 }}f, {{ point.y|floatformat:2 }}f) {% endfor %}};
{% endif %}
{% endfor %}
{% endfor %}
